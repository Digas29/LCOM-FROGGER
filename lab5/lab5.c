...
static int proc_args(int argc, char *argv[]) {
	...
	if (strncmp(argv[1], "test_init", strlen("test_init")) == 0) {
		if (argc != 4) {
			printf("vg:: wrong no of arguments for test_init() \n");
			return 1;
		}
		...
		if (argc != 6) {
			printf(
					"vg:: wrong no of arguments for test_square() \n");
			return 1;
		}
		...
		if (argc != 7) {
			printf("vg:: wrong no of arguments for test_line() \n");
			return 1;
		}
		if ((xi = parse_ulong(argv[2], 10)) == ULONG_MAX)
			return 1;
